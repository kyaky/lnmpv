...
cd ~
wget http://luajit.org/download/LuaJIT-2.0.1.tar.gz -O ~/LuaJIT-2.0.1.tar.gz
...
cd ~
wget http://www.maxmind.com/download/geoip/api/c/GeoIP-latest.tar.gz -O ~/GeoIP-latest.tar.gz
...
cd ~
wget http://nginx.org/download/nginx-1.4.2.tar.gz -O ~/nginx-1.4.2.tar.gz
tar zxvf nginx-1.4.2.tar.gz
cd nginx-1.4.2
pwd=$(pwd -P)
wget https://codeload.github.com/yaoweibin/ngx_http_substitutions_filter_module/zip/master -O $pwd/ngx_http_substitutions_filter_module.zip
wget https://codeload.github.com/simpl/ngx_devel_kit/zip/master -O $pwd/ngx_devel_kit.zip
wget https://codeload.github.com/chaoslawful/lua-nginx-module/zip/master -O $pwd/lua-nginx-module.zip
wget https://codeload.github.com/monadbobo/limit_req2_nginx_module/zip/master -O $pwd/limit_req2_nginx_module.zip
...
#wget http://soft.yzs.me/nginx-lnmpv.conf -O /etc/nginx/nginx.conf
cp -f nginx-lnmpv.conf /etc/nginx/nginx.conf
...
#wget http://soft.yzs.me/fastcgi_params -O /etc/nginx/fastcgi_params
cp -f fastcgi_params /etc/nginx/fastcgi_params
#wget http://soft.yzs.me/nginx -O /etc/init.d/nginx
